...
//				g2.fillRect(j.x, j.y, 100, 100);
	...
	{
		//kills dead bubbles
		ArrayList<Bubble> killTheseBubbles = new ArrayList<>();
		for(GameObject b : bubbles)
		{
			Bubble deadBubble = ((Bubble)b);
			
			if (deadBubble.isDead)
			{
				killTheseBubbles.add(deadBubble);
			}
		}
		bubbles.removeAll(killTheseBubbles);

		//kills dead fruits
		ArrayList<Fruit> killTheseFruits = new ArrayList<>();
		for(GameObject f : fruits)
		{
			Fruit deadFruit = ((Fruit)f);
			
			if (deadFruit.isDead)
			{
				killTheseFruits.add(deadFruit);
			}
		}
		fruits.removeAll(killTheseFruits);

		//kills dead monsters
		ArrayList<Monster> killTheseMonsters = new ArrayList<>();
		for(GameObject m : monsters)
		{
			Monster deadMonster = ((Monster)m);
			
			if (deadMonster.isDead)
			{
				killTheseMonsters.add(deadMonster);
			}
		}
		monsters.removeAll(killTheseMonsters);
		
		//kills dead darts
		ArrayList<Dart> killTheseDarts = new ArrayList<>();
		for(GameObject d : darts)
		{
			Dart deadDart = ((Dart)d);
			
			if (deadDart.isDead)
			{
				killTheseDarts.add(deadDart);
			}
		}
		darts.removeAll(killTheseDarts);
	...
	private void checkCollisions() 
	{
		for (GameObject gameObj : allGameObjectsExceptPlayerAndItsBubble) {
			if (p.pos.x > gameObj.getPosition().x - 20 && p.pos.x < gameObj.getPosition().x + 20 && p.pos.y > gameObj.getPosition().y - 20 && p.pos.y < gameObj.getPosition().y +20) {
		...
		for (GameObject gameObj : monsters) {
			for (GameObject gameBub : tempBubbs) {
				...
				if (temp.pos.x > gameBub.getPosition().x - 20 && temp.pos.x < gameBub.getPosition().x + 20 && temp.pos.y > gameBub.getPosition().y - 20 && temp.pos.y < gameBub.getPosition().y + 20) {
			...
			{
				Monster temp = (Monster) gameObj;
				if (temp.pos.y >= g.getPosition().y && temp.pos.y <= g.getPosition().y + 75 && temp.pos.x <= g.getPosition().x +100 && temp.pos.x >= g.getPosition().x-100) {
					...
					Point tempPt = new Point(temp.pos.x + addOn, temp.pos.y+100);
